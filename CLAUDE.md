...
- `execucao/` — `spec-mvp-v0.md`, `marcos/` (descobertas datadas), `evolucao-para-mauro.md` (reporte sócio), `kanban-legalizai-story-book.md`, `parking-lot.md`, `flow/` (as telas), e as 3 travas de vault (`verificar-tudo`, `verificar-defasagem`, `verificar-autoridade`).
- 🆕 `produto/me/` e `produto/mei/` — **o produto, dividido por regime e depois por ciclo de vida** (reorg de 17/09). Dentro do `me/`: `entrar/{constituir,migrar}` (as duas portas) · `viver/` (o dia-2, e é onde mora o **`motor/`** com `regra · vidas · provar · rodar · publicar · notas`, mais o **`processos/`** com o board e o modo cru) · `desenquadrar/` (saída para EPP) · `devs/` (a entrega gerada, `back/` e `front/`). 🔑 O eixo é o ciclo de vida porque **constituir e migrar são duas portas para a mesma casa, e o motor é a casa** — nenhuma das 18 vidas começa numa tela de abertura. ⚠️ O MEI desenquadrando cai em `mei/desenquadrar/` e aterrissa em `me/viver/`, sem passar por `me/entrar/`: o CNPJ dele já existe. `_raiz.mjs` em `produto/me/viver/` acha a raiz do vault sozinha — **nunca contar `../` à mão**.
