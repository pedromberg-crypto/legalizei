...
for (const busca of process.argv.slice(2)) {
  ...
  // 1. Checar Alias exato
  const aliasMatch = aliases.find(a => limpar(a.termo) === bLimpo);
  if (aliasMatch) {
      const row = dados.find(l => g(l, "codigo").replace(/\D/g, "") === aliasMatch.codigo.replace(/\D/g, ""));
      if (row) {
          console.log(`   🌟 ENCONTRADO POR ALIAS EXATO`);
          console.log(`   1. ${g(row, "codigo")}  ${titulo(row).padEnd(52)} 1.00`);
          continue;
      }
  }

  // 2. Fallback pro pg_trgm (agora ponderando atividades com tsvector se implementássemos, mas aqui é trgm puro ainda)
  const r = dados
    .map((l) => {
        ...
        // mantendo 's' como oficial pra output não ficar bizarro, mas 'w' usa atividades pra passar o filtro
    })
    .filter((x) => x.w >= LIMIAR)
    .sort((a, b) => b.s - a.s)
    .slice(0, 5);
