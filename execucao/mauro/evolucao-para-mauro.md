...
| # | Frente | O que evoluiu | Status |
|---|--------|---------------|--------|
| 1 | 🔥 **Achado grave: a lista de CNAEs** | Fui montar as categorias das telas e descobri que **a nossa lista de "quais atividades a gente atende" nunca foi verificada**. Ela foi montada assim: a Contabilizei **não publica** o que atende, só publica **17 categorias que ela recusa**. A gente pegou essas 17, tirou da lista completa do IBGE, e **presumiu que todo o resto era atendido**. Resultado: o sistema hoje diz que a gente atende **"Defesa", "Justiça", "Relações Exteriores", "Geração de Energia Elétrica" e "Casas de Bingo"**. E até o mapeamento das 17 falhou: **"Cartórios" está escrito na lista de recusados** e mesmo assim saiu como atendido → 🟡 **EM VALIDAÇÃO (Pedro, 15/09).** Vai a parecer técnico interno de especialistas **amanhã, 16/09**. Segue sendo o item mais sério da fila. | 🟡 |
...
| 7 | **O que ficou aberto e é decisão de operação** | Quem é a certificadora parceira, em quanto tempo ela procura o cliente e o que fazer se ele não atende. E a nossa plataforma interna que recebe o certificado dela ainda não existe → ✅ **RESOLVIDO (Pedro, 15/09).** Temos **pelo menos 2 certificadoras parceiras**, e as entregas já estão alinhadas com elas. | ✅ |
...
| 5 | **O que ficou aberto, e é decisão sua com o Pedro** | Quando a gente tirou o certificado digital do plano MEI (R$49), a **fidelidade de 12 meses ficou sem contrapartida escrita** — até então a justificativa era "o certificado vem incluso, pago por nós". O número segue valendo, mas hoje o contrato pediria 12 meses de permanência sem dizer em troca de quê → ✅ **RESOLVIDO (Pedro, 15/09).** A contrapartida da fidelidade no plano MEI é o **desconto nos 3 primeiros meses**, somado à **abertura do MEI gratuita**. | ✅ |
...
| 7 | ⚠️ **A Junta Comercial de MG continua sem sistema automatizado** | Isso atinge justamente os 3 serviços mais caros do catálogo (alteração de contrato, baixa da empresa e mudança de porte). Se continuar assim, esses serviços são feitos à mão, e o preço precisa refletir esse trabalho → 🔴 **SEGUE ABERTO (Pedro, 15/09).** | 🔴 |
...
| # | Frente | O que evoluiu | Status |
|---|---|---|---|
| 1 | 🔴 **Dois prazos correndo, e um é decisão sua** | **30/09** é o último dia para optar pelo chamado Simples Híbrido, que permite apurar os tributos novos (CBS/IBS) fora da guia única e gerar crédito para cliente que é empresa. **01/11** o Emissor Nacional de nota de serviço vira obrigatório para toda empresa do Simples, o que atropela o calendário de BH. O primeiro é decisão de negócio, não de app → 🔴 **SEGUE ABERTO (Pedro, 15/09):** precisamos entender melhor os dois prazos antes de decidir. | 🔴 |
...
| 6 | 🔴 **O primeiro processo desenhado já achou 4 buracos** | Peguei *"cliente pede um serviço avulso e ele entra na fatura"*. Resultado: 5 passos resolvidos, 2 esperando decisão e **4 sem resposta em lugar nenhum** — inclusive **dois que nem o nosso contrato nem o deles respondem** → 🟡 **PARCIAL (Pedro, 15/09), depois de varredura no projeto em 15/09.** ✅ **Cancelamento com serviço em andamento: RESOLVIDO** pela cláusula **12.6** da nossa minuta — *"o Cliente deverá quitar todos os valores em aberto até a data fixada para o encerramento, incluindo faturas de serviços adicionais"*. 🔴 **Seguem sem resposta:** (a) serviço avulso que não pode ser entregue (estorna, vira crédito ou cobra?) e (b) pedido feito com a fatura do mês já fechada. ⚠️ O board tem um passo `P6.5 → P6.6 · mês já fechado`, mas é de **outro processo** (cancelar/corrigir nota fiscal) — palavra igual, assunto diferente, e eu quase marquei como resolvido por causa disso. | 🟡 |
...
| 2 | 🔴 **Achamos e consertamos o maior erro do motor** | O sistema somava o pró-labore de todos os sócios e calculava a guia **como se fosse uma pessoa só**. Numa empresa de 4 sócios recebendo R$3.500 cada, cobrava **R$3.617** onde o correto é **R$1.540** — a tabela do Imposto de Renda é progressiva **por pessoa**, e o teto do INSS também. Atingia 8 das nossas 16 empresas de teste. **Já corrigido, e confirmado por pesquisa em norma primária** | 🟢 |

> 🔢 **Correção de contagem, 17/09.** Esta linha dizia **7** empresas atingidas, e eram **8**. Erro meu de contagem, não de cálculo — o conserto do motor sempre esteve certo, e nenhum valor desta tabela muda. O mesmo "7" estava em mais três arquivos; foi pego por uma trava nova que agora recalcula esses recortes a cada rodada. `[HISTÓRICO]`
