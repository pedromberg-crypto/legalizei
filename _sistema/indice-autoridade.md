...
| 🆕 **CÁLCULO fiscal que roda** (DAS, RBT12, Fator R, DARF do sócio, vencimento, multa) | `execucao/motor-fiscal/apurador.mjs` · as tabelas e constantes da lei em `execucao/motor-fiscal/_tabelas.mjs` | ⚠️ **Precedência 1 aplica aqui com força:** quando o motor e um doc divergirem, **o motor ganha** — ele bate ao centavo contra recibo real. ❌ [[fiscal-simples-bh-2026]] segue mandando na PESQUISA e no que ainda não virou código, mas não no cálculo. ❌ `app/src/lib/fiscal.ts` é **estimador de abertura**, não apurador |
| 🆕 **O que a casa DEVE fazer, quando, e com que dado** (calendário de obrigações do cliente travado) | `execucao/estado-cnpj/ciclo-do-cnpj.mjs` | 🔴 É lista **FECHADA**: obrigação que não está lá **não existe** para o nosso cliente. Quem quiser acrescentar diz de onde vem. ❌ não é o board de processos, que descreve caminho e tela |
| 🆕 **ESTADO de um CNPJ** (o que se guarda × o que se deriva) | `execucao/estado-cnpj/_modelo.mjs` | 🔴 Regra dura: **derivado não se guarda**. Tela que guardar número fiscal próprio é bug dela |
| 🆕 **AJUSTE automático de pró-labore** (o "piloto") | `execucao/motor-fiscal/piloto-pro-labore.mjs` | ⚠️ `proLaboreOtimo`/`naBorda` no `fiscal.ts` miram a margem de 30% (UX-39, recomendação nossa), não o limiar legal — são conselho de interface |
| 🆕 **PERSONAS como empresas vivas** (a vida depois da constituição) | `execucao/estado-cnpj/vidas.mjs` | ❌ `execucao/testes-flutter/personas-entrada-me.md` manda no **dia 1** (entrada no app); as vidas mandam do **mês 1** em diante. Mesmo elenco, superfícies diferentes. ❌ `motor-testes/personas/*.json` é de 16/07 e está morto para isto |
| 🆕 **O que já foi ENCERRADO e não se reabre** | `execucao/motor-fiscal/_encerrados.mjs` | 🔴 `node execucao/motor-fiscal/verificar-encerrados.mjs` derruba a rodada se um assunto encerrado voltar para lista de pendência. Nasceu em 15/09 depois de eu reabrir o mesmo assunto 3× |
| 🆕 **Defeitos achados rodando persona** (com quem pegou, custo e trava) | `execucao/estado-cnpj/_achados-do-motor.md` | ❌ o ledger do Flutter (`docs/achados/` no repo deles) é do **app**, não do motor. Não duplicar |
| 🆕 **O que o elenco de vidas NÃO exercita** | `execucao/estado-cnpj/_cobertura-das-vidas.md` | ⚠️ a régua é *"existe código que nenhuma vida faz rodar?"*, não *"temos personas suficientes?"* |
