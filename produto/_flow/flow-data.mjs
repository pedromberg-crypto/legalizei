...
export const NODES = [
  ...
  /* ─── O CAMINHO MEI, NA ORDEM DO FLOW ──────────────────────────────────
     🔄 07/09 (achado do Pedro: *"como que da M7 eu pulo pra M14?"*) — a
     ordem de DECLARAÇÃO estava embaralhada desde o fork, e ela não é
     detalhe: a fita de pills da `/apresentacao` e a tabela do mapa saem
     daqui. O M12 morava lá em cima com as telas do ME, o M2 vinha depois
     do M11, o bloco da atividade (M7*) ficava colado no M14 e o M13
     aparecia depois da família inteira do 2º pagamento — então, lendo a
     fita, a atividade emendava no certificado e sumiam 6 telas no meio.
     Mesma correção que o ME fez em 02/09 com a C5 pelo mesmo motivo.
     ⚠️ Ordem de leitura = ordem do flow. Nó novo entra no lugar dele. */
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  //
  // ─── 🔒 07/09 — O RAMO VIROU CAMINHO PRÓPRIO (decisão do Pedro) ──────────
  // Até aqui a frase abaixo dizia que o ramo "só bifurca onde a lei obriga", e
  // que E6→E9 e A5 eram compartilhados. Isso deixou de valer pro dinheiro.
  //
  // Motivo: entre 30/08 e 05/09 o caminho ME evoluiu e o MEI herdou 4 defeitos
  // SEM NINGUÉM MEXER NELE — o E8 (contrato) foi eliminado e levou junto as
  // cláusulas próprias do MEI; a A2 (termo) foi aposentada e o MEI passou a
  // cair numa tela de VIABILIDADE, extinta pro MEI pela Res. CGSIM 61/2020; o
  // status virou "fase Junta"; e os splashes de pagamento nunca chegaram nele.
  // Não foi erro de quem mexeu no ME: foi o arranjo de herdar sem fronteira.
  //
  ...
  { id: "M1", caminho: "mei", rota: "/mei/endereco", label: "M1 · Onde você trabalha<br/>(área + endereço)", forma: "tela", classe: "branch", status: "construida", validado: "oficial", falta: "🆕 07/09 — 1ª tela do caminho MEI, no lugar da E3.4 do ME. 3 coisas que a tela do ME tem e esta NÃO tem, todas por lei: (1) **gate de BH** — o ME é limitado a Belo Horizonte porque passa por JUCEMG + prefeitura; o MEI registra pela Redesim e a contabilidade a gente presta de qualquer lugar; (2) **card de endereço fiscal (R$49/mês)** — ele existe pra RESOLVER o gate de BH, e o MEI pode usar o residencial por lei (LC 123 art. 3º-A + Res. CGSIM 22/2010), então seria vender solução pra problema que ele não tem (era o erro nº 1 da auditoria de 28/08); (3) **o aviso de viabilidade** — extinta pro MEI (Res. CGSIM 61/2020), o alvará é dispensado por declaração (erro nº 2 da mesma auditoria). 🆕 pergunta que só existe aqui: a FORMA de endereço (casa · ponto comercial · no cliente · só internet), que interage com a dispensa de alvará. As 3 categorias sem MEI (tech/design/consult, art. 966 CC) aparecem apagadas com o motivo e a porta pro ME", dados: "Categoria de atividade (14, sendo 3 sem MEI) · forma de endereço · CEP + número + complemento" },
  { id: "M2", caminho: "mei", rota: "/mei/impedimentos", label: "M2 · Impedimentos<br/>(no lugar da triagem)", forma: "decisao", classe: "branch", status: "construida", validado: "oficial", falta: "🔄 07/09 (FORK DO RAMO) — saiu de dentro do `/gate` (tela de ME, onde era renderizada por um `if (etapa === 'triagem' && mei)`) e ganhou rota e componente próprios: `/mei/impedimentos`, `components/mei/impedimentos.tsx`. 🆕 28/08 — substitui o E5T no ramo MEI (MEI é unipessoal por definição, art. 966 CC: as perguntas de sócio não existem pra ele). São 3 impedimentos que o PRÓPRIO GOVERNO checa e bloqueia: (1) ser sócio/titular/admin de outra PJ — a RFB cruza o CPF, LC 123 art. 18-A; (2) servidor público federal na ativa — Lei 8.112/90 art. 117; (3) receber aposentadoria por invalidez / salário-maternidade / seguro-desemprego — este NÃO bloqueia, mas a formalização cancela o benefício de forma irreversível, então vira escolha informada com confirmação explícita. Fica ANTES do pagamento pelo mesmo motivo da triagem do ME: não cobramos de quem já sabe que não pode. Componente: `components/mei/impedimentos.tsx` (`ImpedimentosView`)", dados: "Já tem outra empresa? (sim/não) · é servidor federal? (sim/não) · recebe benefício? (sim/não) + ciência explícita se sim" },
  { id: "M2_1", caminho: "mei", rota: "/mei/impedimentos?bloqueio=ja-tem-cnpj", label: "M2.1 · Já tem CNPJ<br/>(bloqueio dentro da M2)", forma: "tela", classe: "saida", status: "construida", validado: "oficial", falta: "Saída de bloqueio do governo, não do produto. Oferece os 2 caminhos reais (baixar a antiga OU abrir como ME) em vez de waitlist — a Legalizai atende essa pessoa hoje, só não como MEI. 🔄 07/09 (fork) — saiu do template compartilhado (`lib/dados-saida.tsx` + `components/saida.tsx`) pro ramo: `components/mei/saidas.tsx`. Motivo não é só isolamento — as saídas do ME são bloqueio de PRODUTO e terminam em waitlist; estas são bloqueio do GOVERNO e terminam oferecendo o ME 🔄 07/09 (pedido do Pedro) — **DEIXOU DE SER TELA.** Virou estado da própria M2, alcançável por `?bloqueio=`, no mesmo movimento que o ME fez em 04/09 com o E6.2 (`/conta?cpf=nome`). A mudança apagou uma repetição real: a M2 já abria o aviso do bloqueio inline e, logo depois, mandava a pessoa pra outra tela dizer a mesma coisa com mais detalhe. Nada foi cortado no caminho — a base legal em card, o texto da alternativa e o **formulário de contato** (o que transforma um bloqueio em lead) desceram pra dentro da M2. O conteúdo segue em `components/mei/saidas.tsx`, que passou a ser DADO e não tela", dados: "Nome + contato" },
  { id: "M2_2", caminho: "mei", rota: "/mei/impedimentos?bloqueio=servidor", label: "M2.2 · Servidor federal<br/>(bloqueio dentro da M2)", forma: "tela", classe: "saida", status: "construida", validado: "oficial", falta: "Vedação do art. 117 da Lei 8.112/90, só pra FEDERAL na ativa. A saída não fecha a porta pra estadual/municipal de propósito: lá a regra vem do estatuto de cada ente e em muitos casos é permitido. 🔄 07/09 (fork) — conteúdo agora em `components/mei/saidas.tsx` 🔄 07/09 (pedido do Pedro) — **DEIXOU DE SER TELA.** Virou estado da própria M2, alcançável por `?bloqueio=`, no mesmo movimento que o ME fez em 04/09 com o E6.2 (`/conta?cpf=nome`). A mudança apagou uma repetição real: a M2 já abria o aviso do bloqueio inline e, logo depois, mandava a pessoa pra outra tela dizer a mesma coisa com mais detalhe. Nada foi cortado no caminho — a base legal em card, o texto da alternativa e o **formulário de contato** (o que transforma um bloqueio em lead) desceram pra dentro da M2. O conteúdo segue em `components/mei/saidas.tsx`, que passou a ser DADO e não tela", dados: "Nome + contato" },
  { id: "M3", caminho: "mei", rota: "/mei/faturamento", label: "M3 · Faturamento<br/>(gate do teto R$81k)", forma: "decisao", classe: "branch", status: "construida", validado: "oficial", falta: "🆕 07/09 — no lugar da E5F do ME, e é a ÚNICA pergunta do ramo que é gate de verdade. Grade própria terminando em R$6.750/mês (R$81.000/ano ÷ 12, LC 123 art. 18-A) — a do ME vai até R$30 mil e ofereceria 4 faixas em que a pessoa já não é MEI. E o sentido é oposto: no ME faturamento NÃO barra ninguém (decisão 01/09, a ideia é acompanhar o crescimento); aqui o teto é condição legal de existir como MEI. Gate resolvido INLINE (mesma doutrina do E3.4.1 e da E5T.1): quem estoura vê ali o que muda e segue como ME num clique, sem tela de 'não atendemos'. ⚠️ NÃO passa pelo splash E5F.1 de propósito — quem cruzou o gate já leu a resposta na própria tela", dados: "Faixa de faturamento mensal (5 opções, a última estoura o teto) OU valor exato · ciência do teto quando perto" },
  { id: "M4", caminho: "mei", rota: "/mei/conta", label: "M4 · Criar conta", forma: "tela", classe: "branch", status: "construida", validado: "oficial", falta: "🆕 07/09 — no lugar do E6. Os campos são quase os mesmos; o SENTIDO não é, e era isso que o erro nº 5 da auditoria de 28/08 pegou: o subtítulo do E6 dizia 'a gente já adianta o que precisa **pra Junta**', e o MEI não vai à Junta. A frase certa vale mais: estes campos são LITERALMENTE os do formulário do Portal. A conferência do CPF com nascimento fica (mesma razão do ME), e no MEI pesa mais: quem tem divergência descobre no meio do registro que ele mesmo está fazendo, sozinho", dados: "Nome · CPF · data de nascimento · WhatsApp · e-mail · senha" },
  { id: "M4_1", caminho: "mei", rota: "/mei/conta?etapa=codigo", label: "M4.1 · Confirmar código", forma: "tela", classe: "branch", status: "construida", validado: "ux", falta: "🆕 07/09 — estado interno da M4, com deep-link próprio (mesmo padrão que o ME adotou em 04/09 pro E6.1). Sem a rota, o estado fica indistinguível no mapa e ninguém consegue revisar. 🔄 07/09 (2ª rodada, pedido do Pedro): a tela era a folha coral do E6 com um campo curto dentro e passou a usar o LAYOUT APROVADO DO E6.1 — é o mesmo mecanismo nos 2 caminhos (validar contato logo depois de criar o acesso). Herdou o aparelho 3D flutuando na sobra da tela, o contador de reenvio de 60s que vira botão 'Reenviar código' ao zerar, o escape hatch de WhatsApp acima do CTA, e o subtítulo que nomeia PRA ONDE cada envio foi (e-mail e telefone em coral, sem quebra no meio do dado). O painel coral saiu de propósito: ele é o vestíbulo da criação da conta (M4), e repeti-lo aqui faria a pessoa achar que voltou. **O código foi de 6 pra 8 dígitos**, igual ao ME: ele sai do mesmo disparo de autenticação, e tamanho diferente por regime seria divergência de sistema, não de produto. Reescrito, não importado (fronteira intacta)", dados: "Código de 8 dígitos (e-mail + SMS)" },
  { id: "M4_2", caminho: "mei", rota: "/mei/conta?etapa=cpf-divergente", label: "M4.2 · CPF não confere", forma: "tela", classe: "saida", status: "construida", validado: "ux", falta: "🆕 07/09 — estado de exceção da M4. Não é saída terminal: o cadastro fica guardado e a pessoa volta depois de atualizar na Receita. A copy explica por que trava (o Portal puxa o nome direto da Receita) em vez de só dizer 'dados inválidos'", dados: "—" },
  { id: "M5", caminho: "mei", rota: "/mei/plano", label: "M5 · Seu plano<br/>(R$49/mês)", forma: "tela", classe: "branch", status: "construida", validado: "pendente", falta: "🆕 07/09 — no lugar do E7. A tela do ME se chama 'a conta da abertura' e soma mensalidade + taxa da Junta (R$281,08). **No MEI não existe nenhum dos dois lados dessa conta**: a abertura é gratuita e o honorário já é zero desde 14/07. Então a tela responde outra pergunta, que é a tese do produto: se abrir é grátis e eu mesmo clico, o que eu estou pagando? Resposta: não errar agora + o ano seguinte. 🔴 O certificado NÃO vem incluso (decisão do Pedro 28/08) e isso é dito aqui com o valor (R$209/ano), em bloco próprio — não como item riscado no meio dos inclusos. 🔴 ABERTO: a fidelidade de 12 meses ficou sem contrapartida escrita quando o certificado saiu do plano (o ADR 04/08 dizia que ela ERA a contrapartida). Copy usa a formulação genérica; precisa de decisão Pedro/Mauro antes de virar cláusula", dados: "—" },
  { id: "M6", caminho: "mei", rota: "/mei/pagamento", label: "M6 · Pagamento + contrato", forma: "tela", classe: "branch", status: "construida", validado: "pendente", falta: "🆕 07/09 — no lugar do E9, e paga uma dívida: em 30/08 o E8 (`ContratoView`) foi eliminado do ME e o aceite desceu pro pagamento. O MEI tinha um E8 PRÓPRIO, com cláusulas próprias, e ele morreu junto — o que sobrou foi a prop de preço (`semTaxaJunta`) e o checkbox genérico. As 2 cláusulas que só existiam no contrato dele voltam aqui: (1) **o que é e o que não é nosso** (não há API nem procuração que permita registrar MEI por terceiro; a senha gov.br é intransferível por Termo de Uso); (2) **o certificado fica por sua conta**. ⚠️ Não existe cláusula de não-reembolso de taxa: não há taxa. Contrato aberto NA TELA, não em link. 🔄 07/09 (pedido do Pedro): o `boleto` voltou pro TIPO `MetodoMei`. O ramo tinha copiado a decisão de 01/09 do ME (só cartão e Pix no card, porque no Asaas o boleto já vem com QR de Pix embutido) mas copiou ERRADO — apagou o boleto do tipo inteiro, não só da seleção. O ME nunca fez isso: mantém `boleto` em `Metodo` e filtra o que aparece na tela, pra as telas de espera seguirem alcançáveis por rota. Sem isso a M6.SB existia e ninguém conseguia chegar nela", dados: "Método (cartão · Pix · boleto, este só por rota) · CPF do pagador · aceite do contrato" },
  { id: "M6_S", caminho: "mei", rota: "/mei/splash-pagamento", label: "M6.S · Splash 'pagamento confirmado'", forma: "tela", classe: "branch", status: "construida", validado: "ux", semVoltar: true, falta: "🆕 07/09 — o MEI volta a ter tela de status depois de pagar. Em 30/08 o ME ganhou os splashes (revogando o 'cartão/Pix pulam direto pro dossiê') e o próprio flow-data registrava que o MEI ficava de fora: 'MEI segue com o comportamento antigo (fora do escopo desta rodada)'. Ele pulava do pagamento direto pra ocupação. A copy é PRÓPRIA: a do ME diz 'com a guia paga, a Junta pode registrar sua empresa', e no MEI não existe guia nem Junta", dados: "—" },
  { id: "M6_SB", caminho: "mei", rota: "/mei/splash-boleto", label: "M6.SB · Splash 'boleto gerado'", forma: "tela", classe: "branch", status: "construida", validado: "ux", semVoltar: true, falta: "🆕 07/09 — copy própria e o motivo é de fundo: no ME a espera do boleto atrasa o registro na Junta; no MEI não atrasa NADA, porque o registro depende do titular, não do pagamento. Deixar a pessoa achando que parou seria criar uma espera que não existe. 🔄 07/09: **deixou de ser órfã** — com o boleto de volta no tipo, `/mei/pagamento?metodo=boleto` chega aqui, e daqui cai na M6.1 (status pendente), não mais direto na M7", dados: "—" },
  // 🆕 07/09 (pedido do Pedro) — A FAMÍLIA DE PAGAMENTO DO MEI FICA IGUAL À DO
  // ME. O ramo tinha só os 2 splashes felizes; faltavam a recusa, a
  // retentativa e os 2 estados de status pós-pagamento. Pedido literal: *"pense
  // que é tudo igual nos 2 fluxos mudando o que será pago"*.
  { id: "M6_SR", caminho: "mei", rota: "/mei/splash-recusado?next=/mei/pagamento%3Fretry%3D1", label: "M6.SR · Splash<br/>pagamento recusado", forma: "tela", classe: "branch", status: "construida", validado: "pendente", semVoltar: true, falta: "🆕 07/09 — o buraco da família de splashes do MEI: ela só sabia dizer que deu certo. Reusa `SplashMensagemView` com `variante=\"recusado\"` (fundo escuro + gradiente coral, ícone x), mesma peça do E9.SR. Transitório e sem CTA: a decisão (outro cartão, trocar pra Pix) é da tela seguinte. Alcançável por `/mei/pagamento?simular=recusa`. ✍️ A copy não culpa a pessoa nem o banco — recusa de cartão tem dezenas de causas e nenhuma chega até aqui como motivo legível", dados: "—" },
  { id: "M6_R", caminho: "mei", rota: "/mei/pagamento?retry=1", label: "M6.R · Pagamento<br/>(nova tentativa)", forma: "tela", classe: "branch", status: "construida", validado: "pendente", falta: "🟠→🟢 07/09 — ESTAVA MEIO-CONSTRUÍDA E INALCANÇÁVEL: o aviso de recusa já existia no componente (prop `recusado`) e a page já lia `?recusado=1`, mas nada no app apontava pra lá, não havia nó no mapa e ninguém conseguia revisar. Agora tem splash que leva até ela. O param virou `?retry=1`, o mesmo do ME: era o único ponto em que os 2 ramos escreviam a mesma ideia com nomes diferentes, e vocabulário divergente entre caminhos faz alguém procurar a tela pelo nome do outro e concluir que ela não existe. **Não zera o que foi preenchido** — quem teve o cartão recusado já está frustrado, refazer o formulário puniria duas vezes", dados: "Método de pagamento (nova tentativa) · CPF já confirmado" },
  { id: "M6_1", caminho: "mei", rota: "/mei/aguardando", label: "M6.1 · Aguardando boleto<br/>cadastro já liberado", forma: "tela", classe: "espera", status: "construida", validado: "ux", semVoltar: true, falta: "🆕 07/09 — TELA NOVA, o par do E9.1 que o ramo não tinha: quem pagava saía do splash direto pra M7, sem nada dizendo o que aconteceu nem o que falta. **Não dá pra reusar o M12**: ele fala de conferência humana em curso, e aqui nada disso começou (faltam 5 telas de cadastro). Hero na mesma anatomia do M12 + timeline dos passos que faltam, DERIVADA da `ESPINHA_MEI` (M7→M11) e não escrita à mão. ⚠️ O boleto NÃO trava o cadastro, e a tela diz isso: no ME a espera atrasa o registro na Junta, aqui não atrasa nada porque o registro depende do titular clicar no Portal. As 2 ações do hero (ver o boleto · adiantar por Pix) são o mesmo par do E9.1, EMPILHADAS e com o valor no 1º chip. 🔄 07/09 (3ª correção do Pedro no mesmo dia: *\"vamos partir novamente para usar a anatomia idêntica\"*): a timeline era uma `<ol>` PLANA de passos, e o status do ME é um **acordeão de blocos** há semanas — contador \"X de Y\" por bloco, borda coral no bloco da vez, o da vez nascendo aberto, e a lista cobrindo a jornada INTEIRA (o E9.1 mostra \"Dados base preenchidos\" já verde: sem isso a tela lê como se a pessoa estivesse no começo depois de ter pago). Portado pro ramo em `components/mei/_timeline-blocos.tsx`, agora peça compartilhada — a raiz das 3 correções do dia não era descuido de cada tela, era eu remontar a anatomia em vez de portá-la. Ganhou também o link \"Tirar uma dúvida no WhatsApp\" no pé, que faltava nas 2 telas de status do ramo. 🔴 UMA DIVERGÊNCIA DELIBERADA: no ME o CTA fica TRAVADO com boleto pendente (`desabilitado: !pago` — lá o dossiê inteiro espera o dinheiro); aqui ele continua ATIVO, porque o registro do MEI depende do titular clicar no Portal, não do nosso processamento. O que espera o boleto é a CONFERÊNCIA do time, e é isso que o item girando diz. Trocar são 2 linhas, se o Pedro preferir espelhar", dados: "—" },
  { id: "M6_1P", caminho: "mei", rota: "/mei/aguardando?pago=1", label: "M6.1P · Status<br/>(pago, via instantâneo)", forma: "tela", classe: "espera", status: "construida", validado: "ux", semVoltar: true, falta: "🆕 07/09 — variante \"pago\" da M6.1, pra quem veio de cartão ou Pix. MESMA tela, MESMO CTA 'Continuar preenchendo': só o topo muda (check verde de plano ativo em vez do pill de compensação, e as 2 ações do boleto somem). Mesma doutrina que separa E9.1 de E9.1P — estado que muda o que a tela mostra e o que ela deixa fazer é nó, não nota", dados: "—" },
  // 🔄 07/09 (pedido do Pedro) — A OCUPAÇÃO VIROU O BLOCO ATIVIDADE. Era 1
  // tela (lista simples com principal e secundárias juntas) e passou a ser o
  // sistema aprovado da C0/C5 do ME: chegada → escolha da principal com slot e
  // cartões → secundárias → splash de fecho. Pedido literal: *"usar o mesmo
  // sistema de escolha de atividade principal e secundária, telas similares,
  // apenas com os filtros do MEI, aparecendo apenas a lista de CNAEs deles"*.
  { id: "M7_0", caminho: "mei", rota: "/mei/atividade?vazia=1", label: "M7.0 · Sua atividade<br/>(chegada)", forma: "tela", classe: "branch", status: "construida", validado: "ux", falta: "🆕 07/09 — estado 'pelado' da M7, espelhando a C0.0: categoria (vinda da M1) e campo de descrição, sem slot nem cartões, porque ainda não há o que mostrar. Mesmo componente, prop `semResultados`. 🔒 Descrever NÃO é obrigatório (mesma decisão de 02/09 do ME): o CTA libera só com a categoria, e quem apertar sem escrever recebe as ocupações da categoria dele. ✍️ Tem uma frase que o ME não precisa: **a lista do MEI é fechada por lei** (Anexo XI). Sem dizer isso, a pessoa procura a atividade dela com as palavras do dia a dia e conclui que a gente não atende — quando o que existe é uma lista oficial com outro vocabulário", dados: "Descrição da atividade (texto livre, opcional) · categoria já vem da M1" },
  { id: "M7", caminho: "mei", rota: "/mei/atividade", label: "M7 · Atividade principal<br/>(Anexo XI + limite interno)", forma: "tela", classe: "branch", status: "construida", validado: "ux", falta: "🔄 07/09 — substitui a tela única `/mei/ocupacao`. Herda o GESTO da C0 inteiro (categoria num `Select` fechado, campo que cresce em vez de rolar, busca que CONGELA o resultado, slot tracejado fora da área rolável, cartões clicáveis, sheet de detalhes, escolher rola de volta pro slot) e troca o MOTOR: no ME o `mapear()` adivinha entre 1332 CNAEs; aqui o universo é uma lista FECHADA de 66 ocupações do Anexo XI, das quais a categoria já recorta um punhado. ⚠️ A busca **ordena, não filtra** — quem descreveu com outra palavra continuaria vendo a ocupação certa, só mais embaixo; filtrar esconderia a resposta de quem não sabe o nome técnico. 🎯 O LIMITE INTERNO (Cosit 27/2021) mora no sheet: a ocupação é mais estrita que o CNAE que ela mapeia, e quem escolhe 'Reparador(a) de bicicleta' não pode consertar moto. 🟢 DECISÃO DO PEDRO (07/09): o sheet MOSTRA o escopo real do CNAE, **rotulado como sendo do CNAE**, com a ressalva grudada acima — esconder empobrece sem proteger, mas lista sem rótulo vira promessa que a Receita não honra. 🚫 Sem etiqueta fiscal nos cartões: MEI é DAS fixo, não tem anexo nem Fator R. 🔄 07/09 (2ª revisão, achado do Pedro comparando os cartões lado a lado): o cartão tinha saído DIFERENTE do da C0 em 4 pontos — pill à esquerda em vez da coluna da direita, \"Ver detalhes\" em linha própria em vez de entrar na linha do código depois de um \" · \", `rounded-2xl` num cartão que vive no meio de formulário (o ME corrigiu isso pra `rounded-md` em 02/09), e a pill dizendo **\"ocupação do Anexo XI\"** em cinza. Esta última o Pedro cortou com todas as letras: *\"a pessoa nem sabe o que é isso e nem precisa saber\"*. A pill responde à pergunta da TELA, e a resposta é **compatível**, no mesmo verde do ME. O Anexo XI continua vivo onde é útil: no sheet, junto do limite interno, na hora em que ele muda o que ela pode fazer", dados: "Ocupação principal (1, do Anexo XI) · descrição livre (só reordena)" },
  { id: "M7_S", caminho: "mei", rota: "/mei/atividade-secundarias", label: "M7.S · Atividades<br/>secundárias (até 15)", forma: "tela", classe: "branch", status: "construida", validado: "ux", falta: "🆕 07/09 — a C5 do ramo, que não existia (as secundárias moravam na mesma tela da principal). 3 diferenças de fundo pro ME: (1) **o limite de 15 é OFICIAL**, da etapa 'Qualificação do Negócio' do Portal, não régua de produto nossa como no ME; (2) **a busca atravessa as categorias** — as sugestões saem da categoria da principal, mas quem faz duas coisas de ramos diferentes (fotógrafo que também dá aula) precisa achar a segunda em outra categoria; (3) **não existe 'muda seu enquadramento'** — o DAS do MEI é fixo, igual pra toda ocupação. O que pode mudar é outra coisa, e a tela avisa: ocupação de comércio, indústria ou transporte intermunicipal gera **Inscrição Estadual automática no SIARE**. 🔄 07/09 (auditoria de anatomia, a pedido do Pedro): a BUSCA estava no topo do corpo e foi pro RODAPÉ, fixa acima do CTA, dentro de um `Campo` com rótulo. Foi decisão do Pedro na C5 em 02/09 pelo motivo oposto ao que eu tinha assumido — no corpo, quem não se encontra nas sugestões precisa ROLAR ATÉ O FIM pra descobrir que existe busca; fixa, ela é uma saída sempre à mão, e os resultados aparecem no corpo, que é onde a lista mora. O rótulo e o placeholder passaram a ensinar as 2 saídas (buscar fora do ramo e buscar pelo número do código): sem dizer, ninguém descobre nenhuma", dados: "Ocupações secundárias (0 a 15, opcional)" },
  { id: "M7_1", caminho: "mei", rota: "/mei/splash-atividades", label: "M7.1 · Splash<br/>'já sabemos o que você faz'", forma: "tela", classe: "branch", status: "construida", validado: "ux", semVoltar: true, falta: "🆕 07/09 — fecho do bloco, espelhando a C5.1. Não confirma UMA escolha: fecha o assunto ATIVIDADE inteiro (principal + secundárias) antes de o cadastro virar a página pros dados pessoais. ✍️ Copy própria — a do ME fala em CNAE, e aqui o que foi escolhido é OCUPAÇÃO; dizer 'achamos seu CNAE' ensinaria a palavra errada, porque o Portal nunca vai mostrar esse termo pra ela", dados: "—" },
  { id: "M8", caminho: "mei", rota: "/mei/titular", label: "M8 · Seus dados<br/>(o titular)", forma: "tela", classe: "branch", status: "construida", validado: "oficial", falta: "🆕 07/09 — no lugar da C1. Não é coleta do zero: nome/CPF/nascimento vieram da M4 (front-load, doutrina RS9 de 28/07) e aparecem TRAVADOS, que é mais honesto que esconder. Acrescenta os 4 campos do formulário oficial que o ME não pede: **RG, órgão emissor, UF e nome da mãe**", dados: "RG · órgão emissor · UF · nome completo da mãe" },
  { id: "M9", caminho: "mei", rota: "/mei/empresa", label: "M9 · Sua empresa<br/>(atuação + baixo risco)", forma: "tela", classe: "branch", status: "construida", validado: "oficial", falta: "🆕 07/09 — no lugar da C4, sem os 2 campos que eram erro real (auditoria 28/08): **índice cadastral do IPTU** (erro nº 3 — o formulário do MEI não pede, ele não vai à Junta, e o gate TRAVAVA o Continuar) e **'é residência de algum sócio?'** (erro nº 4 — MEI não tem sócio). ✅ Capital social FICA: é campo real da etapa 'Qualificação do Negócio' do Portal, sem mínimo legal (pode ser R$1,00), e a tela diz isso. 🎯 A **declaração de baixo risco** é checkbox explícito, não letra miúda: é o ato jurídico que dispensa a licença prévia (Res. CGSIM 51/2019) e transfere responsabilidade pro titular", dados: "Confirmação do endereço · formas de atuação (1+ das 7) · capital social · declaração de baixo risco" },
  { id: "M10", caminho: "mei", rota: "/mei/nome", label: "M10 · Nome da empresa<br/>(gerado por lei)", forma: "tela", classe: "branch", status: "construida", validado: "oficial", falta: "🆕 07/09 — no lugar da C7, e é CONFIRMAÇÃO, não escolha: foi o erro nº 7 da auditoria de 28/08 (a A1 mostrava pro MEI uma 'razão social escolhida'). No MEI a razão social é gerada por lei — nome civil + CPF (Lei 14.195/2021, Res. CGSIM 48/2018). Não há sugestão de nomes, não há 2ª rodada (a C7′ do ME não tem equivalente) e não há consulta de viabilidade de nome. O que existe de escolha é o **nome fantasia**, opcional e sem análise de ninguém", dados: "Nome fantasia (opcional)" },
  { id: "M11", caminho: "mei", rota: "/mei/revisar", label: "M11 · Revisar e autorizar", forma: "tela", classe: "branch", status: "construida", validado: "oficial", falta: "🔴→🟢 07/09 — CONSERTA O DEFEITO MAIS GRAVE DO RAMO. O MEI tinha a própria A2 (`/termo?regime=mei`, variante `mei` inteira escrita e auditada em 28/08). Em 01/09 a A2 do ME foi aposentada (o aceite virou bloco da A1) e a rota morreu; ninguém percebeu que o MEI morava nela. A partir daí, quem fazia o caminho MEI saía do `/revisar` e caía em **`/iniciar-viabilidade`** — tela sobre a consulta prévia de viabilidade, EXTINTA pro MEI pela Res. CGSIM 61/2020. A copy foi recuperada da `TermoView` aposentada antes de ela ser apagada: os 4 passos do que acontece, as **3 declarações do formulário oficial** (a de baixo risco transfere responsabilidade cível e penal), e o cancelamento SEM a cláusula de taxa da Junta (não há taxa)", dados: "Autorização expressa (checkbox)" },
  ...
  { id: "M12", caminho: "mei", rota: "/mei/status", label: "M12 · Status<br/>(pipeline concierge)", forma: "tela", classe: "branch", status: "construida", validado: "oficial", semVoltar: true, falta: "🔴→🟢 07/09 — ESTA TELA ESTAVA INALCANÇÁVEL. O pipeline estava certo desde 28/08, mas em 31/08 o status do ME migrou pra `/aguardando?fase=junta` (fusão A3+E9) e levou a navegação do MEI junto: quem fazia o caminho MEI via **'fase Junta'**, órgão pelo qual ele não passa, e `/painel?regime=mei` virou rota viva só por URL direta. Agora tem rota e componente próprios (`/mei/status`, `components/mei/status.tsx`); `/painel` segue existindo pro Migrar e não muda. Pipeline PRÓPRIO (`ETAPAS_MEI`): recebemos seus dados → time conferindo → próximos passos prontos (CTA) → empresa aberta. ✍️ REGRA DE COPY: nenhuma etapa pode dizer que a Legalizai registra o MEI (não há API nem procuração que permita — ver `abertura-mei-processo.md`). Ficou FORA da fusão A3+E9 de 31/08 de propósito: o MEI não tem dossiê de 9 passos nem etapa de Junta, então fundir as listas não faria sentido. 🔄 07/09 (correção do Pedro: *\"todas são padrão, corrija todas\"*): a timeline era PLANA, com a justificativa de 28/08 de que \"4 etapas não pedem agrupamento\" — e eu repeti essa justificativa quando ele pediu a anatomia idêntica, em vez de aplicar o padrão. Ele estava certo: a decisão de 28/08 é anterior ao acordeão virar o padrão do status no ME. Agora as **3 telas de status do ramo** (M6.1, M12 e os 4 estados do certificado) consomem a MESMA peça, `components/mei/_timeline-blocos.tsx` — então não têm como divergir uma da outra. O agrupamento espelha a fase Junta do ME: o que já passou vira um bloco fechado (\"Informações confirmadas\"), a fase corrente abre", dados: "" },
  { id: "M13", caminho: "mei", rota: "/mei/proximos-passos", label: "M13 · Próximos passos<br/>(a \"cola\")", forma: "tela", classe: "branch", status: "construida", validado: "oficial", falta: "🔄 07/09 (fork) — mesma rota de sempre, só mudou de route group: saiu de `(app)` (onde morava no meio das telas do ME) pro `(mei)`. Nenhum link quebra. 🆕 28/08 — a tela que FECHA o ramo, e existe por razão jurídica, não de UX: como não dá pra registrar pelo cliente, a entrega é o passo a passo com os valores DELE prontos, na ordem dos campos do Portal. Inclui a checagem do nível da conta gov.br (Prata/Ouro obrigatório) e o link pro Portal. ✍️ REGRA DE COPY DURA: nunca dizer 'a gente abre pra você' neste ramo. 🔴 Falta: definir se o 'copiar tudo' vira PDF/WhatsApp; e o M-S é a tela mais cara de evoluir se um dia a automação for possível. Componente: `components/mei/proximos-passos.tsx` (`ProximosPassosMeiView`)", dados: "Confirmação de que a conta gov.br é Prata/Ouro · (devolve o CNPJ gerado)" },
  { id: "M14", caminho: "mei", rota: "/mei/certificado", label: "M14 · Certificado<br/>(gate: libera o app)", forma: "tela", classe: "branch", status: "construida", validado: "oficial", falta: "🔄 07/09 (fork) — rota própria (`/mei/certificado`) e componente próprio (`components/mei/certificado.tsx`). A rota antiga `/certificado?regime=mei` segue viva pro Migrar, que a usa por outro caminho e não muda. 🆕 28/08 (decisão do Pedro) — o MEI passa pelo MESMO gate de certificado do ME, com 2 diferenças: (1) o MOTIVO — no ME o certificado destrava a procuração da assinatura; no MEI não existe assinatura nem procuração de abertura (a abertura DISPENSA certificado, gov.br Prata/Ouro supre), então o que ele destrava é a OPERAÇÃO: puxar guia, FGTS Digital, agir sem pedir senha do cliente toda vez; (2) QUEM PAGA — no ME vem incluso (contrapartida da fidelidade, ADR 04/08); no MEI **não vem**, o cliente providencia. Fica ANTES da A5 a pedido do Pedro: 'tem que ser efetivado antes da pessoa cair pra dentro do app com as funcionalidades, da mesma forma do ME'. 🔴 Consequência aberta: a fidelidade de 12 meses do MEI perdeu a contrapartida escrita — precisa de justificativa nova antes de virar cláusula (Pedro/Mauro) 🔴 07/09 (decisão do Pedro, 2ª rodada) — **VIROU GATE E VIROU COBRANÇA.** Ela dizia \"não é gate, dá pra seguir sem\" e despejava na casa; agora, sem certificado, a Legalizai não libera o acesso total ao app e a jornada PARA no status (M14‴). E o certificado passa a ser cobrado DENTRO do app (M14.P), não mais pago direto na certificadora — o valor segue sendo o dela, repassado sem acréscimo, mas o dinheiro entra por nós, e é por isso que a gente pode dizer \"está pago\" sem depender de aviso de terceiro. As 2 escolhas da tela passam pelo status: \"quero resolver agora\" → M14.P; \"deixa pra depois\" → M14‴. ✍️ A copy do card \"depois\" foi reescrita junto: dizia *\"segue tudo funcionando\"*, e não segue. 🟡 ABERTO: repassar cobrança de terceiro dentro do app pede redação jurídica com o Mauro; a cláusula da M6 e o bloco de preço da M5 já foram alinhados na copy, mas não revisados por ele", dados: "Certificado digital (upload .pfx/.p12 + senha) OU aceite de contato da certificadora parceira" },
  // 🆕 07/09 (pedido do Pedro) — O SEGUNDO PAGAMENTO DO RAMO MEI, espelhando a
  // família da guia da Junta no ME (A3.P/A3.SR/A3.R/A3.PS/A3.PSB/A3′/A3″).
  // Pedido literal: *"a parte do segundo pagamento muda apenas o que seria
  // pago, em ME a taxa da junta e em MEI o certificado digital"*.
  { id: "M14_P", caminho: "mei", rota: "/mei/certificado/pagar", label: "M14.P · Pagar o<br/>certificado digital", forma: "tela", classe: "branch", status: "construida", validado: "pendente", falta: "🆕 07/09 — não é tela nova: é o MESMO `PagamentoMeiView` da M6 no modo `certificado`, como o ME reusa o `PagamentoView` no modo `guia`. Muda o valor, a copy e o que se aceita; método, dados de cobrança e idempotência são idênticos de propósito. ⚠️ O contrato de serviço NÃO reaparece (foi aceito na M6, e não se aceita duas vezes): o que se aceita aqui é a autorização de emissão em nome do titular e o compromisso da videochamada. 🔴 DECISÃO DE 07/09 QUE ISTO IMPLEMENTA: o certificado passa a ser **cobrado dentro do app**, e não mais 'pago direto na certificadora'. O valor segue sendo o dela, repassado sem acréscimo — muda por onde o dinheiro entra, e portanto quem pode dizer 'está pago' sem depender de aviso de terceiro. 🟡 A cláusula do contrato (M6) e o bloco de preço do plano (M5) ainda dizem 'pagos direto lá' e precisam de revisão jurídica com o Mauro", dados: "Método (cartão · Pix · boleto) · dados do titular · autorização de emissão + videochamada" },
  { id: "M14_SR", caminho: "mei", rota: "/mei/splash-recusado?next=/mei/certificado/pagar%3Fretry%3D1", label: "M14.SR · Splash<br/>certificado recusado", forma: "tela", classe: "branch", status: "construida", validado: "pendente", semVoltar: true, falta: "🆕 07/09 — mesma peça da M6.SR, outro `next`. Alcançável por `/mei/certificado/pagar?simular=recusa`.", dados: "—" },
  { id: "M14_R", caminho: "mei", rota: "/mei/certificado/pagar?retry=1", label: "M14.R · Certificado<br/>(nova tentativa)", forma: "tela", classe: "branch", status: "construida", validado: "pendente", falta: "🆕 07/09 — MESMA tela da M14.P com o aviso da recusa no topo, sem zerar o que foi digitado. A autorização de emissão continua obrigatória na retentativa.", dados: "Método de pagamento (nova tentativa)" },
  { id: "M14_PS", caminho: "mei", rota: "/mei/splash-pagamento?next=/mei/status%3Ffase%3Dcertificado%26certificado%3Dpronto", label: "M14.PS · Splash<br/>certificado pago", forma: "tela", classe: "branch", status: "construida", validado: "pendente", semVoltar: true, falta: "🆕 07/09 — reusa o splash da M6.S com `?next` próprio. Volta pro status com a etapa do certificado fechada e a videochamada virando a vez.", dados: "—" },
  { id: "M14_PSB", caminho: "mei", rota: "/mei/splash-boleto?next=/mei/status%3Ffase%3Dcertificado%26certificado%3Dboleto", label: "M14.PSB · Splash<br/>boleto do certificado", forma: "tela", classe: "branch", status: "construida", validado: "pendente", semVoltar: true, falta: "🆕 07/09 — reusa o splash da M6.SB com `?next` próprio. Volta pro status com a etapa **aguardando o boleto compensar**: a emissão não começa antes disso, e a tela diz isso em vez de deixar a pessoa achando que já está andando.", dados: "—" },
  // Os 4 ESTADOS do status na fase do certificado. Mesma doutrina que separa
  // A3′ de A3″ no ME: estado que muda o que a tela mostra e o que ela deixa
  // fazer é nó, não nota.
  { id: "M14_1", caminho: "mei", rota: "/mei/status?fase=certificado&certificado=pendente", label: "M14‴ · Status<br/>(travado no certificado)", forma: "tela", classe: "espera", status: "construida", validado: "pendente", semVoltar: true, falta: "🔴 07/09 — **É AQUI QUE A JORNADA PARA**, e essa é a decisão nova do Pedro: sem certificado a Legalizai não libera o acesso total ao app. Até hoje a M14 dizia 'não é gate, dá pra seguir sem' e despejava na casa. O estado é de quem escolheu 'deixa pra depois'. ✍️ A copy separa as duas coisas com cuidado, porque confundi-las seria mentira: **o CNPJ está ativo e não depende disso** — o que espera é o acesso completo ao app. O CTA é a saída (pagar), não um aviso", dados: "—" },
  { id: "M14_1B", caminho: "mei", rota: "/mei/status?fase=certificado&certificado=boleto", label: "M14″ · Status<br/>(boleto do certificado<br/>compensando)", forma: "tela", classe: "espera", status: "construida", validado: "pendente", semVoltar: true, falta: "🆕 07/09 — pagou por boleto: a etapa segue girando e o CTA fica visível e travado ('Aguardando o boleto compensar'), mesma regra de 31/08 do ME — a pessoa precisa VER que existe um próximo passo, só não pode agir ainda.", dados: "—" },
  { id: "M14_1P", caminho: "mei", rota: "/mei/status?fase=certificado&certificado=pronto", label: "M14′ · Status<br/>(pago, aguardando<br/>a videochamada)", forma: "tela", classe: "espera", status: "construida", validado: "pendente", semVoltar: true, falta: "🆕 07/09 — pagou por cartão ou Pix: a etapa do certificado fecha e a **videochamada de validação** vira a vez. ⚠️ Ela é exigência da certificadora (ICP-Brasil), não nossa, e a copy diz isso — sem essa frase a espera parece burocracia inventada pela Legalizai.", dados: "—" },
  { id: "M14_1L", caminho: "mei", rota: "/mei/status?fase=certificado&certificado=liberado", label: "M14⁗ · Status<br/>(certificado emitido,<br/>app liberado)", forma: "tela", classe: "espera", status: "construida", validado: "pendente", semVoltar: true, falta: "🆕 07/09 — o único estado da fase em que a jornada volta a andar, equivalente do A3′ do ME. Certificado emitido, videochamada passada, e a casa finalmente vira o destino. Mock por query; no app real quem fecha é a confirmação da certificadora.", dados: "—" },
...
export const EDGES = [
  ...
  /* 🔒 07/09 — O FORK. Antes o MEI seguia pro MESMO E3.4 do ME (com
     `?regime=mei`) e só bifurcava na triagem. Agora ele sai do E3.2 direto
     pro caminho dele, e não volta. Ver `app/src/lib/mei-flow.ts`. */
  { de: "E3_2", para: "M1", label: "MEI · caminho próprio", tracejado: true },
  ...
  // ── 🆕 28/08 · RAMO MEI ────────────────────────────────────────────────
  /* ─── A espinha do ramo MEI, em ordem (`ESPINHA_MEI` no código) ───────── */
  { de: "M1", para: "M2", label: "área com MEI" },
  { de: "M1", para: "E3_4", label: "categoria sem MEI · vira ME", tracejado: true },
  { de: "M2", para: "M3", label: "sem impedimento" },
  { de: "M2", para: "M2_1", label: "já tem outra empresa" },
  { de: "M2", para: "M2_2", label: "servidor federal" },
  { de: "M3", para: "M4", label: "dentro do teto" },
  { de: "M3", para: "E3_4", label: "estourou o teto · vira ME", tracejado: true },
  { de: "M4", para: "M4_1", label: "criar conta" },
  { de: "M4", para: "M4_2", label: "CPF não confere", tracejado: true },
  { de: "M4_1", para: "M5", label: "código confirmado" },
  { de: "M5", para: "M6", label: "seguir" },
  { de: "M6", para: "M6_S", label: "cartão ou Pix" },
  { de: "M6", para: "M6_SB", label: "boleto" },
  { de: "M6", para: "M6_SR", label: "recusado" },
  { de: "M6_SR", para: "M6_R", tracejado: true },
  { de: "M6_R", para: "M6_S", label: "passou na 2ª" },
  /* 🔄 07/09 — os splashes não caem mais direto na M7: passam pelo status
     intermediário, que é onde a pessoa vê o que acabou de acontecer e o que
     falta. Cada um cai no SEU estado — e é por isso que são 2 nós, não 1. */
  { de: "M6_S", para: "M6_1P", tracejado: true },
  { de: "M6_SB", para: "M6_1", tracejado: true },
  { de: "M6_1P", para: "M7_0", label: "continuar preenchendo" },
  { de: "M6_1", para: "M7_0", label: "continuar preenchendo" },
  { de: "M7_0", para: "M7", label: "buscar atividade" },
  { de: "M7", para: "M7_S", label: "principal escolhida" },
  { de: "M7_S", para: "M7_1", label: "seguir" },
  { de: "M7_1", para: "M8", tracejado: true },
  { de: "M8", para: "M9" },
  { de: "M9", para: "M10" },
  { de: "M10", para: "M11" },
  { de: "M11", para: "M12", label: "autorizou" },
  { de: "M12", para: "M13", label: "time conferiu", tracejado: true },
  { de: "M13", para: "M14", label: "voltou com o CNPJ", tracejado: true },
  /* 🔄 07/09 — a M14 deixou de despejar na casa. Ela é uma decisão com 2
     saídas, e as duas passam pelo status: quem resolve agora paga, quem
     espera trava. A casa só chega no fim (M14⁗). */
  { de: "M14", para: "M14_P", label: "quero resolver agora" },
  { de: "M14", para: "M14_1", label: "deixa pra depois" },
  { de: "M14_1", para: "M14_P", label: "pagar" },
  { de: "M14_P", para: "M14_PS", label: "cartão ou Pix" },
  { de: "M14_P", para: "M14_PSB", label: "boleto" },
  { de: "M14_P", para: "M14_SR", label: "recusado" },
  { de: "M14_SR", para: "M14_R", tracejado: true },
  { de: "M14_R", para: "M14_PS", label: "passou na 2ª" },
  { de: "M14_PS", para: "M14_1P", tracejado: true },
  { de: "M14_PSB", para: "M14_1B", tracejado: true },
  { de: "M14_1B", para: "M14_1P", label: "boleto compensou", tracejado: true },
  { de: "M14_1P", para: "M14_1L", label: "videochamada validada", tracejado: true },
  { de: "M14_1L", para: "A5", label: "app liberado" },
  ...
  // 🗑️ 07/09 — as 2 arestas que saíam daqui pro MEI (`E9→M_O` e `M_O→C1`)
  // MORRERAM COM O FORK. Elas eram o desenho antigo, em que o MEI atravessava
  // o dinheiro do ME e "reusava o C1"; o comentário que ficava aqui era o
  // registro da dívida ("MEI segue direto por ora, fora do escopo desta
  // rodada"). A dívida foi paga: o ramo tem M6 (pagamento), M6.S/M6.SB
  // (splashes próprios) e M8 (titular) agora.
  ...
  { de: "C1", para: "C2" },
  { de: "C2", para: "C3" },
  // 🆕 04/09 — as 2 variações da C3, alcançadas DENTRO da tela (a lista virou
  // editável): somar sócios até o teto ou remover todos. Tracejadas porque não
  // são passos novos, são a mesma tela em outra anatomia.
  { de: "C3", para: "C3_1", tracejado: true, label: "somou até o teto" },
  ...
  // 🗑️ 07/09 — a aresta `A1→A3_M` saiu com o fork: o MEI não passa mais pela
  // A1 do ME (tem a M11, própria), então não há de onde ela sair.
  { de: "A3", para: "A3_1", tracejado: true, label: "nome recusado" },
  { de: "A3_1", para: "C7_2", tracejado: true, label: "sugerir mais 3" },
  // 🔄 04/09 — o envio passa pelo splash de recibo antes do status.
  { de: "C7_2", para: "C7_2S", tracejado: true, label: "envia os novos nomes" },
  { de: "C7_2S", para: "A3_V", tracejado: true },
  ...
  // 🗑️ 07/09 — as 3 arestas do fim do ramo MEI mudaram de casa: agora vivem
  // juntas com o resto da espinha, lá em cima (M12→M13→M14→A5).
