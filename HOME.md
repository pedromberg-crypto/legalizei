...
> **Última atualização:** 2026-07-15 (fecho motor + fiscal) · Janela nova? rode `/boot`. **Fase:** imersão mês 1 + modelo operacional travado + protótipo/LP + domínios PAGOS + BLOCOS do fluxo (B1+B2+B3) + cobrança (Asaas + cancelamento 4 camadas) + benchmark Padrão R$195 + spec de telas + Lotties organizados + email "teto mínimo" mapeado + INPI aberto + **motor de testes CONSTRUÍDO e rodando** (7 personas, saída em tabela + livro-caixa) + **pesquisa fiscal oficial BH/MG** (2 rodadas salvas; H·Fator R 1º ano e I·migração ficaram vazios → 3ª rodada) + **MVP só serviço** (200 CNAEs de comércio cortados → standby).
...
- 🟢 **Motor de testes — CONSTRUÍDO e rodando** em `execucao/motor-testes/` (Node puro, `node run.js <persona>`, **saída em tabela** #·tela·passo·resultado·OK·**sugestão olhar leigo** + livro-caixa append-only `corrida`+`ajuste`). **7 personas com perfil** (espectro leigo→expert); `reta` e `cida` (7ª, acessibilidade) rodando ✅ PASS. IA de CNAE dublada pela persona (zero token). Falta escalar as 5 (dependem de estender schema p/ B2 inteiro + B3) + add flow #2 (MIGRAR). Spec campo a campo em [[spec-telas-entrada-b1-b2]] (⚠️ **corrigir limite p/ 2 sócios**) · mapa [[mapa-telas-mobile]] · personas [[casos-teste-fluxo-cnae]] → [[2026-07-15-motor-testes-e-pesquisa-fiscal]]
- 🟢 **Pesquisa fiscal oficial (BH/MG 2026)** — 2 rodadas deep-research salvas em [[fiscal-simples-bh-2026]]. **Confirmado:** Fator R parcial por CNAE (3 grupos) · abertura BH ~1 dia (alvará imediato baixo risco) · **DAS automatizável** (API Serpro Integra Contador + procuração e-CAC) · valores 2026 (mín R$1.621 · teto INSS R$8.475,55 · IRRF isento ~R$5k · sublimite MG R$3,6M) · viabilidade nome JUCEMG sem API. **Lacunas (3ª rodada + Larissa):** H·Fator R no 1º ano · I·migração de contador · INSS pró-labore %. Cruzar com Gemini → [[legalize-pesquisa-fiscal-bh-2026]]
...
**Decisões a forçar (semana 2):** **certificado digital** → recomendo começar terceiro (Sete Minas) · **prazo de fidelidade** do plano (12m como o líder? menos, pra vender mais fácil?) → 🟡 Pedro valida mais pra frente. **Travadas 14–15/07:** gateway **Asaas** · cancelamento (4 camadas) · cobrança (abertura grátis + 1ª mensalidade) · **limite sócios = 2** · **endereço fiscal = upsell** · **B3 = 1 plano único (~R$195)** · **MVP só serviço** (comércio cortado) · **sócio no exterior = bloqueio**. **Ainda abertas:** **3ª pesquisa fiscal (H·Fator R 1º ano + I·migração + INSS pró-labore)** + cruzar com Gemini + double-check Larissa · **estender schema motor p/ B2+B3** (acende as 5 personas) + **corrigir spec (limite 2)** · **especificar B4** (constituição — happy path já mapeado) + **B4.5 ativação fiscal/DAS** (API Serpro) · provider API cartão CNPJ + validação CPF · derivados do logo + teste de fogo → Design System · review visual da LP (Pedro) · **registrar marca no INPI + titular** + busca de anterioridade → [[legalize-marca-inpi]].
