...
`produto/_flow/mapa-flow-mermaid.md` é uma nota **GERADA, não editar à mão** (edição some na próxima geração).

**Fonte única:** `produto/_flow/flow-data.mjs` (NODES + EDGES + SUBGRAFOS, com status por nó: `construida/planejada`, `validado: oficial/ux/pendente`, `falta`).
**Gerador:** `node produto/_flow/gerar-mapa.mjs` (criado 2026-07-21). Faz 3 coisas:
