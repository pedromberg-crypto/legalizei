 ...
 *   fala   · com quem: API, órgão, gateway, ou "—" se é só banco nosso
 *            (esta é a ÚNICA coluna técnica, e é técnica de propósito)
 *   ve     · o que a pessoa vê, ou "—" se é invisível (invisível é resposta
 *            válida, mas tem que ser declarada)
...
export const PASSOS = [
  ...
  },
  {
    ...
    fala: "— (banco nosso). 📚 O modelo do líder confirma que fatura suporta itens: `GET /api/pagamentos/faturas/` devolve `itens[]` com `{descricao, valor, tipo}`, e a mensalidade é UM dos tipos, não o objeto.",
  ...
  // ── o fecho ───────────────────────────────────────────────────────────────
  {
    ...
    fala: "— (entra no processo P1, que ainda não foi desenhado)",
