...
export const NOS = [
 ...
 nota: "🔴 BURACO ACHADO EM 12/09, NA NOTA REAL QUE EMITIMOS. A nota saiu com `competencia: 12/09/2026` — a DATA DA EMISSÃO — enquanto a descrição dizia “referente ao serviço prestado no mês de agosto”. 🔑 O texto é decorativo: quem decide em qual apuração a receita entra é o CAMPO competência, e ele foi setembro. O DAS dessa nota vence em outubro, não em setembro. ⚠️ O líder NÃO deixa escolher a competência — usa a data de emissão e pronto. O leiaute nacional TEM o campo (`dCompet`), e a regra E0061 proíbe alterá-lo em substituição para optante do Simples, o que mostra que ele é fiscal de verdade. 🔴 PERGUNTA ABERTA, e é decisão de produto: a gente deixa a pessoa escolher a competência ou copia o líder e trava na data de emissão? Deixar escolher é mais correto e abre a porta pra erro; travar é simples e faz o texto “referente a agosto” mentir para sempre.",
  ...
  { id: "N19", o: "Desistiu: nada é emitido e nada é cobrado", fim: true },
  {
    ...
    saidas: [
      { se: "sim, tudo pelo Simples", vai: "N58" },
 ...
 nota: "🆕 12/09 — o mapa ASSUMIA que existe “o serviço de sempre” (N6) sem ter o passo que cria isso. É a sequência salva, e é o que faz a segunda emissão durar segundos. 🔑 Decisão do Pedro: a gente já nasce melhor que o líder, porque na constituição temos o dossiê completo — então a sugestão vem PRÉ-PREENCHIDA e pertinente, com a pessoa podendo alterar. ⚠️ Ele nunca mexeu nesses campos na Contabilizei em nota nenhuma, o que confirma que pré-preencher certo resolve o caso comum. 🔍 A investigar: o líder cruza a sugestão com o CNAE principal e, provavelmente, com os secundários — mas isso é leitura nossa, não confirmada. Vale abrir antes de copiar a régua. ⚠️ O campo do IndOp NÃO pode ser salvo como favorito no líder, e é justamente o que decide onde o imposto é devido. Se a gente conseguir guardar, é vantagem direta.",
  ...
  {
    id: "N59",
    o: "Pergunta se manda a nota direto pro cliente",
    variavel: "Envia cópia pro cliente?",
    saidas: [
      { se: "sim, manda pro e-mail dele", vai: "N21" },
      { se: "não, eu mesmo mando depois", vai: "N21" },
    ],
    nota: "🆕 12/09 — no líder é um toggle na revisão, JÁ LIGADO, que dispara o comprovante pro e-mail do tomador (e uma cópia pro emitente). O nosso modelo hoje é link pra copiar e mandar na mão. 🔑 Decisão do Pedro: adotar, e como escolha explícita durante a emissão em vez de trabalho depois. ⚠️ Vir ligado por padrão é decisão em aberto: manda e-mail em nome do cliente sem ele pedir.",
  },
  { id: "N21", o: "Transmite a nota ao órgão", saidas: [{ se: "", vai: "N25" }], ja: "P3.5" },
  {
    id: "N25",
    o: "Lê a resposta do órgão",
    variavel: "Como o órgão respondeu?",
    saidas: [
      { se: "processou com sucesso, sem ressalva", vai: "N26" },
      { se: "processou, mas com ressalva na atividade", vai: "N56" },
      { se: "recusou por dado errado", vai: "N27" },
      { se: "não respondeu a tempo", vai: "N28" },
    ],
    ja: "P3.6",
    nota: "🔴 CORRIGIDO EM 12/09 — “aceitou?” era binário, e a resposta do órgão não é. A nota carrega TRÊS SITUAÇÕES INDEPENDENTES, vistas na nota real que emitimos: `situacaoNota` (processou?), `situacaoNFe` e `situacaoCnae` (a atividade da nota está correta?). Na nossa nota vieram “PROCESSADO_SUCESSO” e “Cnae da nota correto” — mas nada impede processar com sucesso E ter ressalva de CNAE. ⚠️ E “não respondeu” continua não sendo “recusou”: tratar igual gera nota duplicada.",
  },
  {
    id: "N56",
    o: "A nota valeu, mas o órgão marcou ressalva na atividade",
    saidas: [{ se: "", vai: "N26" }],
    nota: "🆕 12/09 — a nota existe e produz efeito, mas fica com uma marca de que a atividade declarada não confere. Não é erro de emissão, é risco fiscal guardado: aparece na nota e ninguém é obrigado a olhar. 🔑 Quem não avisa aqui deixa o cliente descobrir na fiscalização.",
  },
  {
    id: "N27",
 ...
 nota: "🔴 A NOTA CONGELA O ENQUADRAMENTO. Visto na nota real de 12/09: o campo `anexoEscolhido` veio com **5**, e as notas de agosto e setembro mostram o mesmo valor — é o Anexo V do Simples gravado NA NOTA, não contagem de anexo. Decisão do Pedro: a gente também grava. 🔑 Sem isso, o dia em que o Fator R virar o histórico passa a mentir sobre qual alíquota valeu em cada competência. 🆕 E o número da nota é atribuído pela Sefin Nacional, não pela casa — a numeração PODE TER PULOS, e o órgão diz que isso “não representa irregularidade fiscal”. A lista não pode alarmar ninguém com buraco de sequência.",
  ...
  // ── B · A NOTA JÁ EXISTE ──────────────────────────────────────────────────
  {
    ...
    saidas: [
      ...
      { se: "emitir outra igual a essa", vai: "N54" },
      { se: "ver o que já aconteceu com ela", vai: "N55" },
      { se: "tem alguma coisa errada", vai: "N23" },
    ],
    cobre: ["3.2", "3.3"],
    nota: "🔒 TRAVADO EM 12/09 (Pedro): são estas QUATRO ações e mais nenhuma. Nota emitida NÃO se edita nem se altera. A decisão bate com o princípio que a própria PBH escreve — *a nota emitida é imutável, ressalvadas as hipóteses de cancelamento ou substituição* — e com o leiaute nacional, onde não existe carta de correção.",
  },
  {
    id: "N54",
    o: "Duplica a nota: nasce uma nova com os mesmos dados",
    saidas: [{ se: "", vai: "N11" }],
    cobre: ["3.1"],
    nota: "🆕 AÇÃO QUE FALTAVA NO MAPA, e ela é a resposta prática pro caso mais comum: o mesmo cliente, o mesmo serviço, todo mês. Duplicar não conserta nada — cria documento NOVO, com número novo. 🔑 Por isso ela cai de volta no caminho de emissão e passa por todas as travas de novo: retenção, teto, regime e o que impede emitir. Duplicata que pula validação é nota errada nascendo rápido.",
  },
  {
    id: "N55",
    o: "Mostra tudo o que já aconteceu com a nota",
    fim: true,
    cobre: ["3.2"],
    nota: "🆕 No modelo nacional isso tem nome: são os EVENTOS vinculados à chave de acesso — cancelamento, substituição, manifestação do tomador, bloqueio do Fisco. Não é log interno nosso, é o que o órgão registrou. 🔑 É também a única janela do cliente pra enxergar que o tomador dele rejeitou ou confirmou uma nota.",
  },
  {
    id: "N31",
  ...
  },
  {
    ...
    nota: "🔑 É o único trecho da categoria que ESPERA — e o prazo NÃO EXISTE: o FAQ oficial da PBH diz que os prazos da análise fiscal “ainda não foram estabelecidos pelo Conselho Gestor do IBS”. Não é lacuna nossa, é fato do órgão, e o produto não pode prometer prazo nenhum. ⚠️ Pode virar mais de um passo.",
  },
  {
    id: "N39",
