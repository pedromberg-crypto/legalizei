...
- `execucao/` — `spec-mvp-v0.md`, `marcos/` (descobertas datadas), `evolucao-para-mauro.md` (reporte sócio), `kanban-legalizai-story-book.md`, `parking-lot.md`, `handoff/`, `portal/`, e as 3 travas de vault (`verificar-tudo`, `verificar-defasagem`, `verificar-autoridade`).
- 🆕 **`produto/` é o produto, dividido por regime e depois por ciclo de vida** (reorg de 17/09). `_`-prefixo = transversal, serve os dois regimes; nome limpo = ramo.
  ...
  - **`produto/me/`** — `PERSONA` e `persona-zero/` (a empresa do Pedro) · `_evidencias/` (teardown da conta do líder: **foto com data, nunca manda em nada**) · `entrar/{constituir,migrar}` (as duas portas) · `viver/` (o dia-2: o **`motor/`** com `regra · vidas · provar · rodar · publicar · notas`, o **`processos/`** com o board e o modo cru, o **`funcionalidades/`** com as 58 core + `specs/`, o **`portal/`** com o inventário de capacidades e o `_historico/` de julho, e as 3 notas de porquê: `_catalogo` · `_matriz-dependencia` · `_mapa-de-cruzamentos`) · `desenquadrar/` (saída para EPP) · `devs/` (a entrega gerada).
  - 🤖 **Doc gerado mora com o gerador.** `PERSONA.md` está em `viver/processos/` (não na raiz) porque quem o escreve é o `gerar-persona.mjs`; `FUNCIONALIDADES.md` em `viver/funcionalidades/`; `_cobertura-das-vidas.md` em `viver/processos/`. A raiz de `produto/` tem **só** `HOME-produto.md` e `_raiz.mjs`.
...
1. **Mapa é espelho da apresentação.** Mesma coleção de telas, duas vistas: uma conectada e em ordem de flow, a outra navegável. Nada existe num sem existir no outro. A coleção única é `NODES` em `produto/_flow/flow-data.mjs`; a fita de pills da apresentação **deriva** dela (`TELAS_DO_FLOW`), não é mais lista escrita à mão.
2. **Tela nova nasce declarada, não copiada.** Um nó no `flow-data` (com `caminho`), o render na apresentação, e a linha no `MOMENTO_POR_NO`. Rodar `node produto/_flow/gerar-mapa.mjs`: mapa, doc do dev e `/conferencia` se atualizam juntos, e a auditoria de espelho avisa o que ficou solto. Sem render, a pill aparece apagada como **"sem tela ainda"** em vez de sumir.
...
2. **E uma linha em `produto/me/viver/portal/portal-data.mjs`**, nos campos `caps` (o que a tela entrega) e `cobre` (que itens do painel de 51 ela cobre). Esse arquivo é a casa; **não criar um segundo inventário**.
...
🔻 **Precedência (Pedro, 11/09):** quem guia é a pesquisa de **setembro** (`produto/` — painel de 51, evidências com API e endpoint). Os quatro docs de **22-23/07**, hoje em `produto/me/viver/portal/_historico/` (matriz, cruzamento, candidatos de home, backlog), são **histórico**: explicam por que as telas nasceram assim, **não decidem o que construir agora**. Não apagar, não obedecer.
...
Régua completa do que conta como capacidade, nomenclatura e o verificador: `produto/_doutrina/_doutrina-capacidades.md`.
