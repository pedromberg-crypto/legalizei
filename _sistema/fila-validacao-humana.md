...
|---|---|---|---|---|
| ~~**Nosso preço**~~ 🟢 **TRAVADO 14/09** | **R$139/mês**, com ofertas de **3 competências** a R$79 (campanha) e R$99 (lançamento), + **12 meses de fidelidade**. O `~R$195 FAKE` morreu. | benchmark do plano Padrão do líder ([[plano-padrao-195-referencia]]) | **Mauro** + custo unitário real (DB+API) | N7 e N9. Se virar parcelamento ou +1 plano, **muda a estrutura** das telas, não só o número → [[legalize-preco-deferido-custo-real]] |
| ~~**DAE JUCEMG**~~ 🟢 **RESOLVIDO 14/09** | **R$281,08** (guia real, print 125) | ✅ já em `fiscal.ts` desde 01/09 | — | ⚠️ **em disputa desde 09/07**: a Izabela cravou **R$288** ([[fluxo-abertura-portais-pedro-dev]]) e nunca foi reconciliado. Aparece na "conta da abertura" (N7) |
...
| ~~**Certificado A1**~~ 🟢 **TRAVADO 14/09** | **R$209**, **terceirizado**. **Incluso no plano ME**; **pago pelo usuário no MEI** | Pedro | ✅ saiu da fila | N7 |
...
| ~~**Consulta de CPF na Receita: preço e limite**~~ 🟢 **TRAVADO 14/09 (Pedro): InfoSimples confirmado**, e o volume de requisições será calculado mais à frente — não bloqueia. | 🕓 Decisão travada 04/09: o CPF é validado contra a Receita na criação da conta (E6) e na saída da C3 (sócios), via **InfoSimples `receita-federal-cpf`** (entrada `cpf` + data de nascimento; volta `nome`, `nome_civil`, `nome_social`, `situacao_cadastral`, `ano_obito`, comprovante). A capacidade está confirmada na doc; **preço por consulta e limite de chamadas não estão na página pública** | **Mauro** (comercial InfoSimples) |
...
| ~~**Reguladas**~~ | 🟢 **RECONFIRMADO 14/09 (Pedro): waitlist, e NÃO atendemos no MVP.** A divergência com o líder (que atende e pede a carteira depois) fica como está | ✅ fechado |
...
| ~~**Certificado digital**~~ | 🟢 **TRAVADO 14/09: terceirizar.** R$209, incluso no ME, pago pelo usuário no MEI | ✅ Pedro |
...
| **`salarioBaseIRRF` vem ZERO em todas as competências**, inclusive nas de R$3.360 — base declarada como zero, em vez de base calculada com imposto zero. É isenção aplicada na origem ou campo não preenchido? | **Mauro** | Se for campo vazio, o nosso motor não pode copiar o comportamento: a base do IRRF alimenta eSocial e DCTFWeb |
| **A `dataAdmissao` do sócio é 01/12/2025, onze dias antes de a empresa existir** (assinatura 11/12, registro 12/12). É escolha deliberada pra fechar a competência de dezembro, ou erro? | **Mauro** | É a data que decide a **competência do primeiro pró-labore** e alimenta o eSocial. Vira a 3ª data que o app precisa guardar |
| 🔴 **Qual o risco concreto de transmitir pró-labore com a qualificação cadastral PENDENTE?** Na conta real: `statusQualifCadastral: "FALTA DADOS"`, PIS não informado, e 9 competências já transmitidas assim | **Mauro** | A DCTFWeb constitui **confissão de dívida irretratável**. E o cliente não sabe: a pendência só aparece no JSON, em tela nenhuma |
