...
| 11/09 | Deixei travas de DADO dentro do `if (PROPOSTAS.length)`. Quando a camada de sugestão esvaziou, elas desligaram **em silêncio** | 🔑 **Trava de dado roda sempre; só comparação de CENÁRIO depende de proposta.** O ponto cego só apareceu quando a camada esvaziou pela primeira vez — condição que nunca tinha acontecido |
