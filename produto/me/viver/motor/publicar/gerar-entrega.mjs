...
/**
 * 🔑 A entrega NÃO sai na pasta do gerador — sai em `produto/me/devs/`.
 *
 * Decidido em 17/09, junto com a mudança do motor. A regra que separa os dois:
 * `motor/publicar/` **escreve**, `devs/` só **recebe**. Assim "gerado × escrito
 * à mão" é uma fronteira de PASTA, não um aviso no topo do arquivo — e o dev
 * tem um endereço só para pegar, sem atravessar o motor.
 */
const DEVS = resolve(RAIZ, "produto", "me", "devs");
...
const destino = (p) => resolve(DEVS, p);
