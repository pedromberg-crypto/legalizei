...
/**
 * 🔑 Um nível, não dois: em 17/09 este script saiu de `execucao/motor-fiscal/`
 * para `execucao/`. Ele não é do motor — audita a etiqueta `autoridade:` de
 * TODO o vault, e ficou junto do `verificar-tudo` e do `verificar-defasagem`,
 * que são as outras duas travas de vault e usam esta mesma convenção.
 */
 ...
 */
const MOTOR = "produto/me/viver/motor";

export const FONTES_QUE_MANDAM = [
  { arquivo: `${MOTOR}/regra/apurador.mjs`, assunto: "o cálculo fiscal que roda" },
  { arquivo: `${MOTOR}/regra/_tabelas.mjs`, assunto: "as tabelas e constantes da lei" },
  { arquivo: `${MOTOR}/regra/piloto-pro-labore.mjs`, assunto: "o ajuste automático de pró-labore" },
  { arquivo: `${MOTOR}/provar/_encerrados.mjs`, assunto: "o que não se reabre" },
  { arquivo: `${MOTOR}/notas/_SUFICIENCIA.md`, assunto: "o que está provado, e por qual prova" },
  { arquivo: `${MOTOR}/vidas/_modelo.mjs`, assunto: "o estado de um CNPJ" },
  { arquivo: `${MOTOR}/vidas/ciclo-do-cnpj.mjs`, assunto: "o calendário de obrigações" },
  { arquivo: `${MOTOR}/vidas/vidas.mjs`, assunto: "as personas como empresas vivas" },
...
/**
 * 🔴 EM 17/09 ESTA LISTA MOSTROU DOIS PROBLEMAS DE UMA VEZ, e o segundo é pior.
 *
 * Na mudança do motor para `produto/me/viver/`, os 11 caminhos aqui ficaram
 * apontando para o endereço antigo. A trava pegou — mas só **3 dos 11**.
 *
 * 🔑 Os outros 8 passaram VERDES por causa do atalho de conferência logo
 * abaixo: ele aceita o caminho **ou** um `[[wikilink]]` com o nome do arquivo.
 * Como o índice cita `[[vidas]]`, `[[apurador]]` e afins, o caminho podre
 * nunca foi olhado. O atalho existe por um motivo legítimo (o índice às vezes
 * cita por wikilink), e o preço dele é este: ele não confere ENDEREÇO.
 *
 * ⚠️ Fica como está, declarado. Quem garante que o endereço existe é a
 * varredura de ausência do `verificar-encerrados`, não esta.
 */
